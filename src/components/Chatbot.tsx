...
type Message =
	...
	| { from: string; type: 'loader' };

const mockMessages: Message[] = [
	{
		from: 'bot',
		text: 'Hi! How can I help you find the best metro path today?'
	}
];
...
export default function Chatbot({
	...
	setEndpoints: (endpoints: { departure: string; destination: string }) => void;
	...
	const handleSend = async (e: React.FormEvent) => {
		...
		try {
			...
			if (data.response) {
				...
				const jsonMatch = trimmedResponse.match(
						...
						if (
							...
							const fetchStopId = async (stationName: string) => {
								const res = await fetch(
									`/api/identity/${encodeURIComponent(stationName)}`
								);
							...
							if (fromId && toId) {
								...
								console.log('Received stop IDs from chatbot:', {
									fromId,
									toId
								});
							} else {
								...
								setMessages((prev) => [
									...
									{
										from: 'bot',
										text: "Désolé, je n'ai pas trouvé une des stations demandées. Merci de vérifier l'orthographe ou d'essayer une autre station."
									}
								]);
							}
							...
							setMessages((prev) => [
								...prev,
								{ from: 'bot', type: 'loader' } as Message
							]);
				...
				setMessages((prev) => [
					...prev,
					{ from: 'bot', text: "Désolé, je n'ai pas compris." }
				]);
			}
		} catch (err) {
			setMessages((prev) => [
				...prev,
				{ from: 'bot', text: 'Erreur lors de la connexion à Gemini.' }
			]);
		}
		setLoading(false);
	};

	function isLoaderMessage(
		msg: Message
	): msg is { from: string; type: 'loader' } {
	...
	function isErrorMessage(msg: Message): boolean {
		return (
			...
			Boolean(msg.text) &&
			msg.text.startsWith(
				"Désolé, je n'ai pas trouvé une des stations demandées"
			)
		);
	}
	...
	return (
		<>
			{!open && (
				...
				>
					<ChatBubbleLeftRightIcon className='size-4 md:size-7 text-black dark:text-white' />
				</button>
			)}
			{open && (
				<div className='fixed w-full h-full max-md:top-0 max-md:left-0 z-50 md:right-5 md:bottom-5 md:w-80 md:max-w-[90vw] md:h-[32rem] bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-fade-in-fast'>
					...
					<form
						onSubmit={handleSend}
						className='flex items-center gap-2 px-4 py-3 bg-white dark:bg-gray-900 border-t border-gray-100 dark:border-gray-700'
					>
						<input
							type='text'
