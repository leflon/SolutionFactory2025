...
if (importArgIndex !== -1 && process.argv.length > importArgIndex + 1)
	importDatasets = process.argv[importArgIndex + 1]
		.split(',')
		.map((s) => s.trim())
		.filter(Boolean);
else if (excludeArgIndex !== -1 && process.argv.length > excludeArgIndex + 1)
	excludeDatasets = process.argv[excludeArgIndex + 1]
		.split(',')
		.map((s) => s.trim())
		.filter(Boolean);
...
/**
 * Normalizes a string, keeping only lowercased letters. Enables a better search experience.
...
if (datasetsToImport.includes('routes')) {
	...
	const rows = query.all();
	for (const row of rows) metroRouteIds.add(row.route_id);
...
if (datasetsToImport.includes('trips')) {
	...
	const rows = query.all();
	for (const row of rows) metroTripIds.add(row.trip_id);
...
if (datasetsToImport.includes('stop_times')) {
	...
	const rows = query.all();
	for (const row of rows) metroStopIds.add(row.stop_id);
